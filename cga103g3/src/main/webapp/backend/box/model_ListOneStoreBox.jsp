...
<!-- ============================================ < Content內容 > ======================================================= -->
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">包廂 /</span> Box Information</h4>
<a class="btn rounded-pill btn-outline-success" href='<%=request.getContextPath()%>/backend/box/model_AddBox.jsp' role="button" aria-expanded="false" aria-controls="collapseExample"><b>新增包廂</b></a>
			...
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/box/box.do">
				...
				<input type="hidden" name="action" value="getOne_StoreBox">
				<input type="submit" value="查詢">
			</FORM>
             
              <!-- Basic Bootstrap Table -->
              <div class="card">
                <h5 class="card-header">${storeID} / 門市包廂</h5>
...
<!-- 						<th>門市編號</th> -->
                ...
                <%@ include file="./file/page1.file"%>
                    <c:forEach var="boxVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
                      <tr>
                        <td>${boxVO.boxID}</td>
<%-- 						<td>${boxVO.storeID}</td> --%>
<td><c:choose><c:when test="${boxVO.boxTypeID == 1}">小包廂</c:when><c:when test="${boxVO.boxTypeID == 2}">中包廂</c:when><c:otherwise>大包廂</c:otherwise></c:choose></td>
                        ...
                        <td><div class="dropdown"><button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button><div class="dropdown-menu"><a class="dropdown-item" href="<%=request.getContextPath()%>/box/box.do?boxID=${boxVO.boxID}&action=getOne_For_Update"><i class="bx bx-edit-alt me-1"></i>修改</a><a class="dropdown-item" href="<%=request.getContextPath()%>/box/box.do?boxID=${boxVO.boxID}&action=delete"><i class="bx bx-trash me-1"></i>刪除</a></div></div></td>
                      </tr>
                    </c:forEach>
                    </tbody>
                  </table>
