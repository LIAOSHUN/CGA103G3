	...
	<ul>
		<c:forEach var="message" items="${errorMsgs}">
			<li style="color:red">${message}</li>
                  ...
                  <div class="card mb-4">
                    <h5 class="card-header"><span class="text-muted fw-light"><b><%=boxVO.getBoxID() %> /</b></span>包廂資訊更改</h5>
                    <!-- Account -->
                   <form id="formAccountSettings" method="POST" ACTION="<%=request.getContextPath()%>/box/box.do" enctype="multipart/form-data">
                    <div class="card-body">
                      <div class="d-flex align-items-start align-items-sm-center gap-4">
                        <img
                          class="show"
                          src="<%=request.getContextPath()%>/BoxImg?BoxID=${boxVO.boxID}"
                          height="150"
                          width="150"
                        />
                        <div>
                          <label for="upload"  tabindex="0">
                            <span class="d-none d-sm-block">包廂圖片</span>
                    ...
                    <div class="card-body">
                        <div class="row">
                          <div class="mb-3 col-md-6">
                            <label for="firstName" class="form-label">包廂編號</label>
                            <input class="form-control" value="<%=boxVO.getBoxID()%>" readonly />
                          </div>
                          <div class="mb-3 col-md-6">
                            <label for="firstName" class="form-label">門市編號</label>
                            <input class="form-control" value="<%=boxVO.getStoreID()%>" readonly />
                          </div>
                          
                         <div class="mb-3 col-md-6">
                            <label name="boxTypeID" for="language" class="form-label">包廂類型</label>
                            <select name="boxTypeID" id="boxTypeID" class="select2 form-select">
                              <option value="<%=boxVO.getBoxTypeID() %>" selected="selected" hidden><%=boxVO.getBoxTypeVO().getBoxName() %></option>
                              <option value="1" >小包廂</option>
                              <option value="2" >中包廂</option>
                              <option value="3" >大包廂</option>
                            </select>
                          </div>
                          
                          <div class="mb-3 col-md-6">
                            <label name="boxCapcity" for="boxCapcity" class="form-label">包廂人數</label>
                            <select name="boxCapcity" id="boxCapcity" class="select2 form-select">
                              <option value="<%=boxVO.getBoxCapcity()%>" selected="selected" hidden><%=boxVO.getBoxCapcity()%> 人以下</option>
                              <option value="4" >4 人以下</option>
                              <option value="8" >8 人以下</option>
                              <option value="12" >12 人以下</option>
                            </select>
                          </div>

                          <div class="mb-3 col-md-6">
                            <label for="boxPrice" class="form-label">包廂價格</label>
                            <input
                              class="form-control"
                              type="text"
                              id="boxPrice"
                              name="boxPrice"
                              value="<%=boxVO.getBoxPrice()%>"
                            />
                          </div>

                          <div class="mb-3 col-md-6">
                            <label for="boxDescription" class="form-label">包廂說明</label>
                            <textarea
                              type="text"
                              class="form-control"
                              id="boxDescription"
                              name="boxDescription"
                            ><%=boxVO.getBoxDescription()%></textarea>
                          </div>

                          <div class="mb-3 col-md-6">
                            <label for="boxBkStart" class="form-label">包廂預約起始時間</label>
                            <select name="boxBkStart" id="boxBkStart" class="select2 form-select">
                              <option value="<%=boxVO.getBoxBkStart() %>" selected="selected" hidden><%=boxVO.getBoxBkStart() %>:00</option>
                          ...
                          <div class="mb-3 col-md-6">
                            <label for="boxBkEnd" class="form-label">包廂預約結束時間</label>
                            <select name="boxBkEnd" id="boxBkEnd" class="select2 form-select">
                            <option value="<%=boxVO.getBoxBkEnd() %>" selected="selected" hidden><%=boxVO.getBoxBkEnd() %>:00</option>
                              <option value="0">00:00</option>
                              <option value="1">01:00</option>
                              <option value="2">02:00</option>
                              <option value="3">03:00</option>
