...
<a class="btn rounded-pill btn-outline-success" href='<%=request.getContextPath()%>/backend/bookingorder/model_GetBookingStauts.jsp' role="button" aria-expanded="false" aria-controls="collapseExample"><b>未結束訂單</b></a>
           ...
           <jsp:useBean id="storeSvc" scope="request" class="com.store.model.StoreService" />
			...
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/bookingorder/bookingorder.do">
				<b>查詢單筆訂位訂單:</b>
				<input type="text" name="BookingID" value="" placeholder="輸入訂單編號" maxlength="5" size="11" autocomplete="off">
                ...
                <div class="table-responsive text-nowrap">
                  <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                      <tr>
                        <td>${bokOrdVO.bookingID}</td>
						<td>${bokOrdVO.memID}</td>
						<td>${bokOrdVO.bookingDate}</td>
						<td>${bokOrdVO.bookingStart}</td>
						<td>${bokOrdVO.bookingEnd}</td>
						<td>${bokOrdVO.boxVO.storeID}</td>
						<td>${bokOrdVO.boxID}</td>
						<td>${bokOrdVO.bookingNote}</td>						
						<td>${(bokOrdVO.bookingStatus == 1)? '<span class="badge bg-label-success">訂位成功</span>':'<span class="badge bg-label-dark">訂位結束</span>'}</td>
                        <td><div class="dropdown"><button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button><div class="dropdown-menu"><a class="dropdown-item" href="<%=request.getContextPath()%>/bookingorder/bookingorder.do?bookingID=${bookingOrderVO.bookingID}&bookingStatus=0&&action=finish_BoookingOrder"><i class="bx bx-check me-1"></i>結束訂單</a><a class="dropdown-item" href="<%=request.getContextPath()%>/bookingorder/bookingorder.do?bookingID=${bookingOrderVO.bookingID}&action=getOne_For_Update"><i class="bx bx-edit-alt me-1"></i>修改</a><a class="dropdown-item" href="<%=request.getContextPath()%>/bookingorder/bookingorder.do?bookingID=${bookingOrderVO.bookingID}&action=delete"><i class="bx bx-trash me-1"></i>刪除</a></div></div></td>
                      </tr>
                    </tbody>
                  </table>
                </div>
