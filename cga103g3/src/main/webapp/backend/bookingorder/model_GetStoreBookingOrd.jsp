<%@page import="com.bookingset.model.BookingSetService"%>
<%@page import="com.bookingset.model.BookingSetVO"%>
...
<!-- == Leo ================================================================================================= -->
            <li class="menu-item open active">
              ...
              <ul class="menu-sub">
                ...
                <li class="${storeID==51001? 'menu-item active':'menu-item'}">
                ...
                <li class="${storeID==51002? 'menu-item active':'menu-item'}">
                ...
                <li class="${storeID==51003? 'menu-item active':'menu-item'}">
                ...
                <li class="${storeID==51004? 'menu-item active':'menu-item'}">
		...
		<c:forEach var="message" items="${errorMsgs}">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">門市訂位訂單 /</span> BookingOrder Information</h4>
<a class="btn rounded-pill btn-outline-success" href='<%=request.getContextPath()%>/backend/bookingorder/model_AddBookingOrder.jsp' role="button" aria-expanded="false" aria-controls="collapseExample"><b>新增訂位訂單</b></a>
<a class="btn rounded-pill btn-outline-success" href='<%=request.getContextPath()%>/backend/bookingorder/model_GetBookingStauts.jsp' role="button" aria-expanded="false" aria-controls="collapseExample"><b>未結束訂單</b></a>
          ...
          <jsp:useBean id="storeSvc" scope="request" class="com.store.model.StoreService" />
			<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/GetStoreBookingOrd/GetStoreBookingOrd.do">
              ...
              <div class="card">
                <h5 class="card-header">${storeID} / 門市訂位訂單表</h5>
            ...
            <c:forEach var="bokSetVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
                      <tr>
                        <td>${bokSetVO.bookingID}</td>
						<td>${bokSetVO.memID}</td>
						<td>${bokSetVO.bookingDate}</td>
						<td>${bokSetVO.bookingStart}</td>
						<td>${bokSetVO.bookingEnd}</td>
						<td>${bokSetVO.storeID}</td>
						<td>${bokSetVO.boxID}</td>
						<td>${bokSetVO.bookingNote}</td>
						<td>${(bokSetVO.bookingStatus == 1)? '<span class="badge bg-label-success">訂位成功</span>':'<span class="badge bg-label-dark">訂位結束</span>'}</td>
                        <td><div class="dropdown"><button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button><div class="dropdown-menu"><a class="dropdown-item" href="<%=request.getContextPath()%>/bookingorder/bookingorder.do?bookingID=${bokSetVO.bookingID}&bookingStatus=0&&action=finish_BoookingOrder"><i class="bx bx-check me-1"></i>結束訂單</a><a class="dropdown-item" href="<%=request.getContextPath()%>/bookingorder/bookingorder.do?bookingID=${bokSetVO.bookingID}&action=getOne_For_Update"><i class="bx bx-edit-alt me-1"></i>修改</a><a class="dropdown-item" href="<%=request.getContextPath()%>/bookingorder/bookingorder.do?bookingID=${bokSetVO.bookingID}&action=delete"><i class="bx bx-trash me-1"></i>刪除</a></div></div></td>
                      </tr>
                    </c:forEach>
                    </tbody>
                  </table>
