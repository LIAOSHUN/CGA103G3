...
//		}
        ...
        if ("registerInsert".equals(action)) { // 來自addEmp.jsp的請求  
			...
			req.setAttribute("errorMsgs", errorMsgs);
				...
				String memAccount = req.getParameter("memName");                                //使用者名稱
				...
				/***************************2.開始新增資料***************************************/
				MemberService memberSvc = new MemberService();
				memberVO = memberSvc.addMemberRegister(memName, memAccount, memPassWord, memGender, memPh, memEmail,memAddress,memBirthday,memCard);
